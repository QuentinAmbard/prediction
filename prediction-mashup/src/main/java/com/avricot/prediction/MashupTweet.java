...
public class MashupTweet {
	...
	private void mashup(long midnight) {
		...
		for (Candidat candidat : candidats) {
			 ...
			 List<Tweet> tweets = tweetRepository.findByCandidatNameAndBetween(candidat.getCandidatName(), startDate, endDate);
			 TweetReport dailyTweetReport = new TweetReport();
			 int negativeTweets = tweetRepository.findByCandidatNameAndBetweenAndPolarity(candidat.getCandidatName(), startDate, endDate, Polarity.NEGATIVE).size();
			 int positiveTweets = tweetRepository.findByCandidatNameAndBetweenAndPolarity(candidat.getCandidatName(), startDate, endDate, Polarity.POSITIVE).size();
			 
			 /* Score =  tweets de la polarité / nombre de tweet total */
			 int tweetNumber = tweets.size();
